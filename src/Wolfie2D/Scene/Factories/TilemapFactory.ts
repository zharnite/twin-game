...
export default class TilemapFactory {
  ...
  add = (key: string, scale: Vec2 = new Vec2(1, 1)): Array<Layer> => {
    ...
    for (let layer of tilemapData.layers) {
      ...
      if (isParallaxLayer) {
        sceneLayer = this.scene.addParallaxLayer(
          layer.name,
          new Vec2(1, 1),
          depth
        );
      ...
      if (layer.type === "tilelayer") {
        ...
        if (isNavmeshPoints) {
          ...
          this.scene
            ...
            .addNavigableEntity(navmeshName, new Navmesh(g));
        ...
        for (let obj of layer.objects) {
          ...
          for (let tileset of tilesets) {
            if (tileset.hasTile(obj.gid)) {
              ...
              sprite.position.set(
                (obj.x + size.x / 2) * scale.x,
                (obj.y - size.y / 2) * scale.y
              );
          ...
          if (!sprite) {
            for (let tile of collectionTiles) {
              if (obj.gid === tile.id) {
                ...
                sprite = this.scene.add.sprite(imageKey, layer.name);
                sprite.position.set(
                  (obj.x + tile.imagewidth / 2) * scale.x,
                  (obj.y - tile.imageheight / 2) * scale.y
                );
                sprite.scale.set(scale.x, scale.y);
              }
            }
          ...
          if (hasPhysics) {
            ...
            if (isTrigger && triggerGroup !== null) {
              let groups: string[] = triggerGroup.split(/[ ,]+/);
